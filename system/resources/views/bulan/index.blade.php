...
@section('PageTitle', 'Master Bulan')
@section('header')
<!-- Bagian Header -->
        ...
        <div class="col-lg-12">
          ...
          <div class="card">
            ...
            <div class="row">
                ...
                <?php
                if($all_access->where('name','Bulan-Add')->count() > 0){
                    ?>
            ...
            <div class="table-responsive">
                ...
                ?>
                    <tr>
                      ...
                      <td>{{$d->Singkatan}}</td>
                      <td>
                      <?php if($all_access->where('name','Bulan-Edit')->count() > 0){ ?>
                      <button type="button" data-toggle="modal" data-target="#edit{{$d->Singkatan}}" class="btn btn-warning btn-xs waves-effect waves-light"><i class="fa fa-edit"></i> Edit</button>
                      <!-- Modal Edit -->
                      <div class="modal fade" id="edit{{$d->Singkatan}}">
                        ...
                        <div class="modal-content border-secondary">
                            ...
                            <div class="modal-body">
                                <form action=<?= route('Bulan.Update'); ?> method="post">
                                <input type="hidden" name="id" value="{{$d->Bulan_Id}}">
                                @csrf
                                    <div class="input-group mb-3">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text">Nama Bulan</span>
                                    </div>
                                    <input id="Nama_Bulan" type="text" class="form-control @error('Nama_Bulan') is-invalid @enderror" name="Nama_Bulan" value="{{$d->Nama_Bulan}}">
                                    </div>
                                    @error('Nama_Bulan')
                                    ...
                                    <div class="input-group mb-3">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text">Bulan ID</span>
                                    </div>
                                    <input type="text" id="Bulan_Id" class="form-control  @error('Bulan_Id') is-invalid @enderror" name="Bulan_Id" value="{{$d->Bulan_Id}}">
                                    </div>
                                    @error('Bulan_Id')
                                        <p style="color:red;font-size:9px;margin-left:100px;">{{ $message }}</p>
                                    @enderror
                      ...
                      <?php } ?>
                      <?php if($all_access->where('name','Bulan-Delete')->count() > 0){ ?>
                      <a href="<?= url('bulan/delete/'.$d->Bulan_Id);?>" class="btn btn-danger btn-xs waves-effect waves-light"><i class="fa fa-trash"></i> Delete</a>
                      <?php } ?>
                      </td>
                    </tr>
                    <?php endforeach;?>
...
<!-- Modal -->
<?php if($all_access->where('name','Bulan-Add')->count() > 0){ ?>
    ...
    <div class="modal-content border-secondary">
        ...
        <div class="modal-body">
            <form action=<?= url('bulan/create'); ?> method="post">
            @csrf
            <div class="input-group mb-3">
            <div class="input-group-prepend">
                <span class="input-group-text">Nama Bulan</span>
            </div>
            <input id="Nama_Bulan" type="text" class="form-control @error('Nama_Bulan') is-invalid @enderror" name="Nama_Bulan"  value="{{ old('Nama_Bulan') }}">
            </div>
            @error('Nama_Bulan')
                <p style="color:red;font-size:9px;margin-left:100px;">{{ $message }}</p>
            @enderror
            ...
            <input type="text" id="Singkatan" class="form-control  @error('Singkatan') is-invalid @enderror" name="Singkatan" value="{{ old('Singkatan') }}">
            </div>
            @error('Singkatan')
                <p style="color:red;font-size:9px;margin-left:100px;">{{ $message }}</p>
            @enderror
            ...
            <input type="text" id="Bulan_Id" class="form-control  @error('Bulan_Id') is-invalid @enderror" name="Bulan_Id" value="{{ old('Bulan_Id') }}" >
            </div>
            @error('Bulan_Id')
                <p style="color:red;font-size:9px;margin-left:100px;">{{ $message }}</p>
            @enderror
